---
name: ito-search
description: 處理專案外部的查詢需求：套件用法與 API、錯誤訊息、GitHub issue／PR、方法論與一般知識。由 agent 依 query 自選工具並過濾劣質網域，輸出附可點擊來源 URL 與使用工具標註，供使用者驗證。以「幫我查」、「搜尋一下」等自然語觸發。不適用於 codebase 搜尋、需直接實作的任務、需產出研究報告或需存檔的調研任務。
---
