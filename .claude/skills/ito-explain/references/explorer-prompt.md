...
**你僅回結構化 finding，不輸出任何 diagram（ASCII 架構圖、sequence 圖等皆不輸出）**。Diagram 由後續的 explainer 以全局視角集中畫完，避免 ASCII 圖在 synthesize 階段需要重畫對齊。將你蒐集到的元件、依賴、flow 步驟以下列格式回傳，供 explainer 消費後產圖。

## 輸出格式

以以下結構回傳 findings。以事實與具體資訊為主——引用確切檔案路徑、函式名稱、型別名稱，必要時加行號。**不產出任何 diagram**。

### 結構化 Finding Schema

供 explainer 直接消費以繪製架構圖與 sequence。以 yaml-like 格式回傳，actors 與 steps 的命名精確、一致，供 explainer 繪圖時直接套用：
...
- 模組: <module name>
  檔案:
    ...
    - <path>:<line>
  對外依賴:
    - <module or external name>
    - <module or external name>
  負責 flow:
    - name: <flow name>
      actors: [<A>, <B>, <C>]           # 依訊息流順序列
      steps:
        - "1. <A>→<B>: <msg>"           # 編號 + 來源→目標: 訊息
        - "2. <B>→<C>: <msg>"
      notes:
        - "<side effect 描述，例：persist to localStorage>"
        - "<分支條件，例：if 401 then ...>"
```

**Flow 粒度原則：** 一個 flow 描述一條從觸發到效果的完整路徑。不同觸發點（例：登入、登出、401）各自獨立為一個 flow。actors 列盡量壓在 3–5 個以內，若同模組內多個細步驟可合併為單一 actor。

### 找到的元件
列出關鍵型別、服務、類別、抽象。每項：名稱、檔案路徑、一句話說明用途。

### 執行流程
逐步描述執行流程。每步：哪個函式 / 方法、所在檔案、做什麼、呼叫誰。加註步驟間流動的資料。此段為散文／條列，給 explainer 補充結構化 finding 不易涵蓋的脈絡。
...
### 值得注意之處
意外、歷史遺留、易誤解之處。看似該這樣運作但實際另一回事的地方。每條註記可附建議 tag（例：`[安全]`、`[同步]`、`[未用]`、`[版本]`、`[文件]`、`[效能]`、`[相容]`），供 explainer 撰寫「眉角」時採用。
