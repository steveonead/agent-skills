...
Spawn 單一唯讀子 agent，在同一輪中完成探索與撰寫解釋。該 agent 讀取 `references/explainer-prompt.md` 取得輸出格式與溝通風格（該 prompt 自會要求讀取 `references/output-format.md` 擷取樣式規範）。不需 reconcile 多來源 findings——它的 findings 就是它自己的探索結果。所有 diagram 由該 agent 集中產出，採先彙整結構化 finding 後再畫的順序，不邊探索邊畫。
...
1. Explainer 讀取 `references/explainer-prompt.md` 取得完整 prompt 模板、輸出格式與溝通風格（該 prompt 自會要求讀取 `references/output-format.md` 擷取視覺化樣式規範）。
2. **Diagram 集中產出：** 所有 diagram 由 explainer 一手畫完。Explorer 僅回結構化 finding（見 `references/explorer-prompt.md` 內的 finding schema），不輸出任何 diagram——避免 ASCII 圖在 synthesize 階段需要重畫對齊。
3. **矛盾處理：** Explainer 遇到 findings 矛盾時，能用自身讀碼能力仲裁者，整合為統一敘述，並於對應段落引用仲裁所用檔案路徑與行號作為可驗證痕跡；無法仲裁者，於「眉角」段落明列為「注意：此處不同路徑觀察結果不一致，可能反映 X」。
4. **Gap 處理：** Explainer 有唯讀權限補探 findings 中標註的 Open Questions。若 gap 仍無法解決，於最終輸出誠實揭露。
...
Explainer 輸出採下列順序段落結構。**TL;DR 與架構圖為預設必列**；其餘段落依題目性質可省略（例：runtime trace 題常不需「關鍵概念」；架構鳥瞰題無歷史背景可談則略「眉角」）。

1. **TL;DR**：1 行模組定位 + 3 bullet（架構 / 關鍵機制 / 最大坑）。**bullet 3 強制為 risk**；無可談 risk 則整段略去，不以全正向形式輸出。
2. **架構圖**：ASCII 單張。Swimlane 優先（技術分層 ≤ 4 欄），Layered box fallback。
3. **概覽**：1–2 段。此 subsystem 是什麼、做什麼、為何存在。
4. **關鍵概念**：核心型別 / 服務 / 抽象，簡要定義，僅列理解後續所需。
5. **運作方式**：解釋核心，走一次流程。散文為主，不塞 pseudocode 或大段程式碼；引用具體檔案路徑與函式名稱。達門檻的 flow 加 ASCII sequence 圖（門檻與樣例見 `references/output-format.md`）。
6. **檔案位置**：簡短檔案 / 目錄圖，只列動工時會碰到的。
7. **眉角**：值得注意之處、陷阱、歷史脈絡、矛盾 findings。**每條 bullet 前加 inline tag**（例：`[安全]`、`[同步]`、`[未用]`、`[版本]`、`[文件]`）。

**產出密度要求：** 每段至少引用 1 個具體檔案路徑或函式名稱，避免空泛架構圖解。

**樣式細節與門檻：** TL;DR 格式、架構圖 Swimlane / Layered box 規則、sequence 圖門檻與 `[note]` 用法、眉角 tag 分類清單皆見 `references/output-format.md`。explainer 與 Simple 路徑單 agent 撰寫輸出前讀取該檔以擷取樣式規範。
...
| 「每條 flow 都畫 sequence 比較完整」 | 無門檻篩選會稀釋 signal、稀釋視覺 anchor 的效益；sequence 上限 2 張是為了強迫挑最有價值的畫（通常錯誤路徑或跨層授權） |
| 「TL;DR 跟概覽重複，跳過 TL;DR 直入概覽」 | TL;DR 的 3 bullet 分層（架構 / 機制 / 坑）是 readability 的核心入口；省略會回到原本資訊密度爆炸的問題 |
| 「Explorer 順手畫圖可以省 synthesize 階段重畫」 | ASCII 圖在 synthesize 階段合併等於重畫；explorer 僅回結構化 finding 才能讓 synthesizer 以全局視角一次畫對 |
...
- TL;DR 缺項或 3 bullet 未涵蓋 risk（bullet 3）
- 輸出缺架構圖，或 sequence 圖超過 2 張
- 眉角 bullet 無 inline tag（[安全]／[同步] 等）
- Explorer 回傳結果含 diagram（explorer 僅回結構化 finding）
...
- [ ] 結果僅對話呈現，未寫入檔案
- [ ] TL;DR 含 1 行定位 + 3 bullet，bullet 3 為 risk（或整段略去）
- [ ] 架構圖為 ASCII 且為 Swimlane（≤4 欄）或 Layered box fallback
- [ ] Sequence 圖不超過 2 張，每張符合 actor ≥ 3 / 跨檔案 ≥ 3 / 有分支 任一門檻
- [ ] 眉角每條 bullet 前含 inline tag
- [ ] Explorer 輸出不含任何 diagram（diagram 由 explainer 集中畫）
...
- `references/explorer-prompt.md`：Complex 路徑 explorer 子 agent 的完整 prompt 模板，含結構化 finding schema。
- `references/explainer-prompt.md`：Simple 與 Complex 路徑 explainer 子 agent 的完整 prompt 模板。
- `references/output-format.md`：TL;DR、架構圖、sequence、眉角 tag 的樣式規範與 ASCII 樣例。
