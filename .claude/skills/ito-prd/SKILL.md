---
name: ito-prd
description: 將使用者需求訪談後收斂為結構化 PRD，產出單一 PRD 主 issue（不拆 sub-task），支援新增與編輯模式，最後存至 local 或建立 gh issue。使用者說「寫 PRD」、「整理需求成 PRD」、「開需求 issue」、「編輯既有 PRD」時使用。不適用於需要直接實作、純技術架構設計、或非需求文件的一般討論。
---
...
**不應使用的情況：** 需要直接實作功能、純粹技術架構設計討論、非需求文件的一般筆記、需求尚未成形需要先發散討論、或需要將 PRD 拆成 sub-task（改用 `/ito-issues`）。
...
依決策樹的每個分支逐步深挖，先解決有依賴關係的決策。訪談聚焦產品層面（使用者、情境、痛點、效益、邊界），可帶到技術討論，但不深入實作細節。
