...
- 有 `<issue-number>` argument → 進入**修訂模式**，跳至「修訂模式」章節
- 無 argument 或 argument 不是 `<issue-number>` → 進入**訪談模式**，繼續步驟 2

### 步驟 2：偵測 Context

檢查當前 conversation 是否包含需求或需求 summary：
...
2. **User Stories**：行為級別，每條附編號（US-XX）、簡短標題、主述句（「身為 … 想要 … 以便 …」），以及驗收條件清單（每條以「前提 / 動作 / 結果」三段陳述）。US 只描述使用者行為，不需考量 vertical slice 切法。
...
> 訪談已收斂，PRD 內容已備妥。請選擇輸出方式：
>
> 1. 存入本地 `docs/prd/{slug}.md`，先 review 暫時不建立 Issue
> 2. 直接建立 GitHub Issue，不存本地檔案
>
> 請回覆 1 或 2。

- 選擇 1 → 進入**步驟 6**（本地存檔），流程結束
- 選擇 2 → 進入**步驟 7**（直接建立 Issue），流程結束

兩條路徑共用：讀取 `assets/prd-template.md`，提取各 section 結構與 placeholder 格式，依訪談結果填入各 section。所有文字使用繁體中文台灣用語，proper nouns 保留英文。
...
> PRD 已存至 `docs/prd/{slug}.md`，請 review 後決定是否建立 Issue。
>
> 後續若要建立 Issue，可手動執行：
> `gh issue create --title "[PRD] {功能名稱}" --label "PRD" --body-file docs/prd/{slug}.md`

**不建立 GitHub Issue**，流程結束。

### 步驟 7：直接建立 GitHub Issue（選項 2）

**不建立本地檔案**。將 PRD markdown 寫入暫存檔以避開 heredoc 跳脫問題，再以 `--body-file` 建立 Issue：

```bash
# 將 PRD markdown 寫入 $TMPDIR/prd-{slug}.md
bash .claude/skills/ito-prd/scripts/validate-prd.sh "$TMPDIR/prd-{slug}.md"
```

若驗證失敗，依 stderr 錯誤補齊缺漏 section 後重新寫入暫存檔。
...
gh issue create \
  --title "[PRD] {功能名稱}" \
  --label "PRD" \
  --body-file "$TMPDIR/prd-{slug}.md"
rm "$TMPDIR/prd-{slug}.md"
```

建立成功後，回報 Issue URL。流程結束。
...
- 一回合出現兩個以上問題
- User Stories 只有功能級別（「使用者可以登入」），缺乏行為細節與「前提／動作／結果」三段驗收條件
- PRD 沒有 Out of Scope section
- 遇到技術問題自行查驗 codebase，而非問使用者或標記 Open Question
- Out of Scope 完全靠使用者主動說，沒有即時追問
- 步驟 5 未詢問選項、自行決定存 local 或直接建 Issue
- 選項 1 流程同時建立本地檔案與 GitHub Issue，或選項 2 流程寫入 `docs/prd/`
...
- [ ] 每條 User Story 都有編號（US-XX）、簡短標題、主述句（身為／想要／以便）與「前提／動作／結果」三段驗收條件
- [ ] 問題描述三項（情境／痛點／目標）回答模糊時都有深追
- [ ] Out of Scope：訪談中每個提到的功能都有即時確認，收斂前有統一盤點
- [ ] 已知侷限：效能／第三方依賴／時程已盤點
- [ ] 步驟 5 已詢問使用者選擇輸出方式（1：本地存檔 / 2：直接建立 Issue），未逕自二擇一
- [ ] 若選擇 1：`docs/prd/{slug}.md` 已建立且通過 `validate-prd.sh`，且**未**建立 GitHub Issue
- [ ] 若選擇 2：Issue title 以 `[PRD]` 開頭，label 含 `PRD`，且**未**建立本地檔案
