...
**不應使用的情況：** 需要直接實作功能、純技術架構討論、PRD 尚未定案需要先跑 ito-prd、或 PRD 已在進行中不應重新切分。
...
**不寫任何 code**，產出只保留在 agent 的內部 context 中，不寫入檔案。本步驟目標：
...
3. 識別現有 pattern、convention、dependency、風險、待釐清事項

**完整性檢查：**

- 若 PRD 缺驗收條件、缺 User Stories、或需求敘述過於發散，soft warn 使用者指出具體缺口，由使用者決定繼續執行或退回 `/ito-prd` 補齊
- 若 PRD 只涉及單一層（純 CSS、純 config、純文件），soft warn「看起來不需要 vertical slicing」，使用者可 override 後仍進行拆分（拆成 commit-size task）
...
2. **複述 diff list** 給使用者確認：「我理解你的意思是以下 N 項修改，確認嗎？」
3. 使用者確認後才套用，**依最終順序從 1 重新連號**（例如原本的 5a 變成 `[PRD-36/4]`），套用後重新展示完整 breakdown 回到步驟 6
...
3. 依 dependency 順序建立 issue（blocker 先建立，才能在後續 issue body 與 dependency 關係中引用真實 issue number）
4. 對每個 slice：
   - 建立 issue，title 格式 `[PRD-<parent-number>/<final-index>] <純裸標題>`（例如 `[PRD-36/1] 建立廣告操作後台登入流程`）
...
**工具選擇：** 本步驟的所有動作（建立 issue、建立 label、建立 sub-issue 關係、建立依賴）由 agent 自行選擇適當工具完成（`gh` CLI、`gh api`、`gh api graphql`、REST API 皆可），skill 不綁定特定工具或參數格式。
...
| 「dependency graph 內部想過就好，不用真的推」 | 略過步驟 4 會導致 slice 依賴順序錯、blocker 晚於 blockee 被建立 |
| 「這個 slice 雖然是 horizontal，但工作量剛好」 | 工作量大小不是判準；horizontal slice 無法獨立 demo，後續整合風險會集中爆發 |
| 「使用者大致同意了，不用展示 diff list」 | 自由文字意圖容易誤讀，跳過複述確認會讓修改錯位而使用者不察 |
| 「approval 後再 prompt 一次比較安全」 | 已決議步驟 8 建完不再 prompt；多一次摩擦違反設計，若有誤需整批關閉後重新執行 |
| 「既有 sub-issue 直接忽略，追加新的就好」 | 違反 re-run 規則，會產出重複任務與混亂依賴關係 |
...
- [ ] 步驟 7 的修改都經過 diff list 複述後才套用，且最終已重新連號
...
- 若步驟 3 探索失敗（repo 結構異常、權限問題），停下並回報具體錯誤，不強行進入 slicing 階段
- 若使用者 iteration 時的自由文字無法 parse 成明確 diff（例如只說「改好一點」），追問具體想改什麼，不貿然套用
...
- `references/re-run-precheck.md`：重新執行時的既有 sub-issue 偵測與批次關閉流程
