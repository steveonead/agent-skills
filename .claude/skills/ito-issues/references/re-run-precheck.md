...
1. **該 PRD 是否已有 sub-issue**
   ...
   - 只計入 open 狀態的 sub-issue（已 closed 的視為歷史，不阻擋重新執行）
...
> - ...
>
> 依你之前的設計決策，若要修改 breakdown 需要全部關閉後重新建立。要繼續嗎？
>
> - A）關閉舊 sub-issue（close reason: `not_planned` 並附註 `superseded by ito-issues re-run`）後重新執行
> - B）取消本次執行
...
- **不自動處理已 closed 的 sub-issue**：已 closed 的視為歷史，重新執行時只在新 sub-issue 的 body 中不引用它們即可

## 邊界情境

- **使用者重新執行時 PRD title 改過了**：sub-issue title 前綴仍用 PRD issue number（例如 `PRD-36`），不隨 title 變動；新 sub-issue 的 index 從 1 重新連號
