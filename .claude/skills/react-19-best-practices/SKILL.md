...
description: React 19 最佳實踐規則集，供撰寫、審查或重構 React 19 元件程式碼時參考。明確要求 Vite + React 19（無 SSR）、搭配 TanStack Query 與 TanStack Router 的 SPA 環境。適用於新功能開發、PR self-review、Class component → Function component 或 React 18 → 19 migration。不適用於需要 SSR / RSC、Next.js App Router、純 Server Component 設計、或 TanStack Query / Router 自身的使用慣例。
---

# React 19 Best Practices

針對 Vite + React 19 + TanStack 系列的 SPA 環境，整理出 React 本身的高 ROI 規則。專注於 React 元件設計、Hook 使用、Effect 管理、效能與安全議題，不重複 ESLint 已能完全覆蓋的基本檢查。
...
- `component-variant-over-boolean` — 變體元件取代 boolean prop 堆疊
- `component-compiler-memo` — React Compiler 自動處理 memoization，禁止手寫 `useMemo` / `useCallback` / `React.memo`
