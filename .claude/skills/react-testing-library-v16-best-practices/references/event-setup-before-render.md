...
# userEvent.setup() 在 render 前建立並複用

> 每個測試先 `const user = userEvent.setup()` 再 render，整個測試共用同一個 user 實例，不要每次互動都重新 setup。
...
- eslint 的 `prefer-user-event` 只要求你「用 userEvent 而非 fireEvent」，不管你有沒有正確 setup 與複用，所以這條要自己守。
